...
pthread_t datadog_watchdog_thread;
...
invoke_previous_handler(int signum, siginfo_t *info, void *user_context) {
...
void *override_native_signal_handlers() {
    pthread_mutex_lock(&mutex);
...
bool start_watchdog_thread() {
    ...
    if (pthread_sigmask(SIG_BLOCK, &signals_mask, NULL) == 0) {
        ...
        if (pthread_create(&datadog_watchdog_thread, NULL,
                           override_native_signal_handlers, NULL) != 0) {
            success = false;
